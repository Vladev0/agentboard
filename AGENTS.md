...
- Суть задачи изменилась по итогам шага → `update_description` с резюме. Просто продвинулись
  рутинно (статус, поле) → `update_status`/`update_fields`-эквивалент, апдейт не нужен, это
  сама система логирует в активность.
...
- Не оставляйте задачу в обычном рабочем статусе — иначе `get_next_task` будет предлагать её
  снова и снова, и цикл забуксует. Заведите в проекте статус вроде `Needs Input`/«Ждёт
  человека» (это обычная колонка канбана, настраивается в `project.md → statuses`, никакого
  особого кода не требует) и переведите задачу туда через `update_status`.
...
Большинство MCP-совместимых агентов используют одинаковый формат конфига:
